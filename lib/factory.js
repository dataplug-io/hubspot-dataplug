...
const { JsonSequentialStreamsReader } = require('@dataplug/dataplug-json');
...
function declareOutputFactory(uri, selector = '!.*', mapping = config.mapping) {
  return (params) => {
    ...
    return new JsonSequentialStreamsReader(
      (factoryParams, previousData) => new Promise((resolve, reject) => {
        if (previousData && !previousData['has-more']) {
          reject(null);
          return;
        }

        return Promise.all([uri, selector, mapping]).then(([uri, selector, mapping]) => {
